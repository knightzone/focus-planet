...
> 2026-09-20 分类变更：28 个游戏采用观察发现、记忆辨识、反应控制、逻辑空间、切换兼顾（6/5/6/5/6）。服务端按 gameId 更新归属。项目未上线，不兼容旧分类、不增加报告版本门禁；测试库可清空重建，联调时同时重置旧客户端测试缓存。本次未执行清库。完整映射见 [分类落地说明](TRAINING_CATEGORIES_V2.md)，不表示后端已部署。
