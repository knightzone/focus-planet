// 听声捉图形（原听声找图；坐标轴四区 + 语音指令）的浏览器近似预览：直接跑真实页面模板与脚本，仅用于视觉与手感检查。
// 用法：node tools/preview-listen-find.cjs
//   http://127.0.0.1:4182/                     规则/试玩（进页面就是可点试玩）
//   http://127.0.0.1:4182/?start=1&level=6     直接开始并指定难度
//   http://127.0.0.1:4182/?start=1&auto=1      自动点出唯一答案（跑完整局）用来目检 6 轮换题
//   http://127.0.0.1:4182/?debug=1             打印场地与图形实际尺寸，复核小屏（配合 ?w=&h=）
...
function render(url) {
  ...
  const page = read('pages/game/listen-find.uvue')
  ...
  const template = page.slice(page.indexOf('<template>') + 10, page.lastIndexOf('</template>'))
    ...
    .replace(/<TrainingFrame[^>]*>/, `<div class="frame"><div class="training-status" style="height:${topInset}px"></div><div class="nav">‹　听声捉图形</div>`)
    .replace('</TrainingFrame>', '</div>')
  const common = strip(read('utils/listen-find-shapes.uts')) + '\n' + strip(read('utils/listen-find-voice.uts'))
  const autoScript = auto ? `setInterval(()=>{
    if(!started.value||field==null) return;
    for(const shape of field.shapes) if(!shape.tapped&&listenFindMatches(shape,field.command)) tapShape(shape.id);
  },60);` : ''
...
const uni={getWindowInfo:()=>({windowWidth:${width},windowHeight:${height},statusBarHeight:${topInset},pixelRatio:3}),
  ...
  createInnerAudioContext:()=>{const audio={src:'',play:()=>{audio.played=true},stop(){},pause(){},onPlay(){},onEnded(){},offPlay(){},destroy(){}};return audio},
...
${autoScript}
${debug ? `setTimeout(()=>{const lines=['innerWidth='+window.innerWidth+' innerHeight='+window.innerHeight,'场地='+fieldWidth.value+'x'+fieldHeight.value+' 每区图形='+level.value+' 指令='+commandText.value];for(const selector of ['.frame','.lf-page','.lf-header','.lf-command','.lf-field','.lf-axis-h','.lf-shape','.lf-mark','.lf-stats','.lf-start']){const element=document.querySelector(selector);if(!element){lines.push(selector+' MISSING');continue}const rect=element.getBoundingClientRect();lines.push(selector+' xy='+Math.round(rect.x)+','+Math.round(rect.y)+' '+Math.round(rect.width)+'x'+Math.round(rect.height))}showBootError(lines.join(String.fromCharCode(10)))},900);` : ''}
return {${names.join(',')}}
}});
...
app.mount('#app');`
  ...
  const chrome = `*{box-sizing:border-box}body{margin:0;background:#dfe7ee;font-family:Arial}#app{width:${width}px;max-width:${width}px;height:${height}px;margin:auto}div{display:flex;flex-direction:column;min-width:0}span{display:block}img{object-fit:contain;flex-shrink:0}button{border:0;flex-shrink:0}.frame{height:100%}.nav{height:44px;flex-shrink:0;background:#eaf3fb;padding:10px 16px;color:#2f6f92}.lf-scene{object-fit:cover}`
  return '<!doctype html><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>听声捉图形预览</title><style>' + chrome + page.match(/<style>([\s\S]*?)<\/style>/)[1] + '</style><div id="app">' + template + '</div>' + errorOverlay + '<script src="/vue.js"></script><script>' + boot + '</script>'
}
...
}).listen(4182, '127.0.0.1', () => console.log('http://127.0.0.1:4182/?start=1&level=6'))
