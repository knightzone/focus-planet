// 规律小侦探的浏览器近似试玩：直接跑真实页面模板与脚本，仅用于视觉检查，不写账号或成绩。
// 用法：node tools/preview-pattern-find.cjs
//   http://127.0.0.1:4179/            试玩页（含“先试一下”演示）
...
function render(url) {
  ...
  const page = read('pages/game/pattern-find.uvue')
  ...
  const template = page.slice(page.indexOf('<template>') + 10, page.lastIndexOf('</template>'))
    ...
    .replace(/<TrainingFrame[^>]*>/, '<div class="frame"><div class="nav">‹　规律小侦探</div>')
    .replace('</TrainingFrame>', '</div>')
  const common = ['local-test', 'pattern-find-levels'].map(name => read('utils/' + name + '.uts').replace(/^import .*$/gm, '').replace(/^export /gm, '')).join('\n')
...
${auto ? `setInterval(()=>{if(!started.value){if(demoSolved.value)begin();else answerDemo(demoOptions[0]);return}if(feedback.value=='')answer(current.value.answerId)},250);` : ''}
return {${names.join(',')}}
}});
app.component('Actionsurface', Vue.defineComponent({ name: 'ActionSurface', template: '<div><slot /></div>' }));
app.component('ActionSurface', Vue.defineComponent({ name: 'ActionSurface', template: '<div><slot /></div>' }));
...
app.mount('#app');
${debug ? `setTimeout(()=>{const lines=['innerWidth='+window.innerWidth+' innerHeight='+window.innerHeight];for(const selector of ['.pf-content','.pf-header','.pf-sequence','.pf-tile','.pf-options','.pf-option','.pf-option-image','.pf-feedback','.pf-stats']){const element=document.querySelector(selector);if(!element){lines.push(selector+' MISSING');continue}const rect=element.getBoundingClientRect(),style=getComputedStyle(element);lines.push(selector+' '+Math.round(rect.width)+'x'+Math.round(rect.height)+' bg='+style.backgroundColor)}showBootError(lines.join(String.fromCharCode(10)))},700);` : ''}`
...
}).listen(4179, '127.0.0.1', () => console.log('试玩页 http://127.0.0.1:4179/　直接开局 http://127.0.0.1:4179/?start=1&level=1'))
